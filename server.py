...
# rooms[code] = {
#   players: [ {'sid':..., 'name':..., 'secret':..., 'secretSet':False}, ... ],
#   game: 'ludo'|'chess',
#   state: {...},
#   host: sid
# }
...
def init_ludo_state(code):
    pl = rooms[code]["players"]
    order = [p["sid"] for p in pl]
    tokens = {}
    for p in pl:
        tokens[p["sid"]] = [{"steps": -1} for _ in range(4)]
    return {
        "tokens": tokens,
        "order": order,
        "turnIndex": 0,
...
def move_ludo_token(state, sid, token_index, dice):
    ...
    if t["steps"] < 0:
        ...
        if t["steps"] >= TOTAL_STEPS:
            ...
            board_idx = steps_to_board_index(start, t["steps"])
            if board_idx is not None:
                if board_idx not in SAFE_SQUARES:
                    for other_sid, oth_tokens in state["tokens"].items():
                        ...
                        for ot in oth_tokens:
                            if 0 <= ot["steps"] < MAIN_LEN:
                                ...
                                if other_board == board_idx:
                                    ot["steps"] = -1
        return True

def check_ludo_winner(state):
    for sid, toks in state["tokens"].items():
        if all(t.get("steps", -1) == 999 for t in toks):
            return sid
    return None
...
def apply_chess_move(state, sid, frm, to, players):
    ...
    piece = brd[r1][c1]
    if not piece:
        return False, None, "no piece"
    color = piece[0]
    mycolor = state["color_of"].get(sid)
    if mycolor != color:
        return False, None, "not yours"
    if state["turn"] != color:
        return False, None, "not your turn"
    ptype = piece[1]
    target = brd[r2][c2]
    if target and target[0] == color:
        return False, None, "own capture"
    dr = r2 - r1
    dc = c2 - c1
    legal = False
    if ptype == "P":
...
def on_start(data):
    code = data.get("room")
    if code not in rooms:
        emit("start_denied", {"msg": "Room not found"}, room=request.sid)
        return
